...
export async function getUsers(): Promise<User[]> {
  const { data, error } = await supabase
    .from('users')
    .select('id,username,name,email,role,company_id,position_id,user_group_id,hire_date,resignation_date,is_active,is_executive,is_leave_exempt,resident_registration_number,created_at,updated_at')
...
export async function addUser(
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      resident_registration_number: user.resident_registration_number || null,
      is_active: true,
      created_at: now,
      updated_at: now,
    }])
    .select('id,username,name,email,role,company_id,position_id,hire_date,resignation_date,is_executive,resident_registration_number,is_active,created_at,updated_at')
    .single();
...
export async function updateUser(
  ...
  if (updates.resignation_date !== undefined) payload.resignation_date = updates.resignation_date;
  if (updates.is_executive !== undefined) payload.is_executive = updates.is_executive;
  if (updates.resident_registration_number !== undefined) payload.resident_registration_number = updates.resident_registration_number;
