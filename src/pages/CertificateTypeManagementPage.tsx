...
export default function CertificateTypeManagementPage() {
  ...
  const openForm = (type?: CertificateType) => {
    if (type) {
      setFormData({
  ...
  const handleSave = async () => {
    try {
      ...
      if (formView?.id) {
        await updateCertificateType(formView.id, submitData);
      } else {
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    {formView !== null ? (formView.id ? '증서 유형 수정' : '증서 유형 추가') : '증서 유형 관리'}
                  </CardTitle>
                  <p className="text-xs text-muted-foreground mt-1">
                    {formView !== null ? '증서 유형 정보를 입력하세요' : '선원 증서 카테고리 및 유형 관리'}
                  </p>
                </div>
              </div>
              {formView !== null ? (
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {typesInCategory.map(type => (
                                <TableRow key={type.id} className="cursor-pointer hover:bg-gray-50" onClick={() => openForm(type)}>
