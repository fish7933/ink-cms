...
import type { AllowanceTemplate } from '@/types/allowance';
import { useTabContext } from '@/contexts/TabContext';
import AllowanceTemplateAssignmentsSection from '@/components/allowance/AllowanceTemplateAssignmentsSection';
import { usePermissions } from '@/hooks/usePermissions';

export default function AllowanceTemplatesPage() {
  const navigate = useNavigate();
  const { openNewTab } = useTabContext();
  const { toast } = useToast();
  const permissions = usePermissions('allowance_templates');
  const [templates, setTemplates] = useState<AllowanceTemplate[]>([]);
  const [loading, setLoading] = useState(true);

  const [renewTarget, setRenewTarget] = useState<AllowanceTemplate | null>(null);
  const [renewDate, setRenewDate] = useState('');
  const [renewing, setRenewing] = useState(false);

  const [copyTarget, setCopyTarget] = useState<AllowanceTemplate | null>(null);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      setTemplates(await allowanceService.getTemplates());
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const openEdit = (t: AllowanceTemplate) => openNewTab(`/allowance-templates/${t.id}/edit`, `템플릿 수정: ${t.name}`);

  const openAssign = (t: AllowanceTemplate) => {
  ...
  const performDelete = async (ids: string[]) => {
    await Promise.all(ids.map(id => allowanceService.deleteTemplate(id)));
  ...
  const requestDelete = async (ids: string[]) => {
    ...
    try {
      const summary = await allowanceService.getTemplateAssignmentSummary(ids);
      if (summary.total === 0) {
  ...
  const openRenew = (t: AllowanceTemplate) => {
    setRenewTarget(t);
    setRenewDate(new Date().toISOString().slice(0, 10));
  };

  const openCopy = (t: AllowanceTemplate) => {
    setCopyTarget(t);
  ...
  const handleCopy = async () => {
    ...
    setCopying(true);
    try {
      const newTemplate = await allowanceService.copyTemplate(copyTarget.id, copyName.trim());
      if (!newTemplate) {
        toast({ title: '복사 실패', description: '템플릿을 복사하는 중 오류가 발생했습니다.', variant: 'destructive' });
        return;
      }
      toast({ title: '복사 완료', description: '항목 구성이 그대로 복사되었습니다. 필요한 금액을 수정해주세요.' });
      window.dispatchEvent(new CustomEvent('allowance-template-data-changed'));
      setCopyTarget(null);
      openNewTab(`/allowance-templates/${newTemplate.id}/edit`, `템플릿 수정: ${newTemplate.name}`);
    } finally {
      setCopying(false);
    }
  };
  ...
  const handleRenew = async () => {
    ...
    setRenewing(true);
    try {
      const newTemplate = await allowanceService.renewTemplate(renewTarget.id, renewDate);
      if (!newTemplate) {
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">수당/공제 템플릿 관리</CardTitle>
                {permissions.canCreate && (
                  <Button size="sm" className="gap-1.5 h-8" onClick={() => openNewTab('/allowance-templates/new', '수당/공제 템플릿 추가')}>
            ...
            <CardContent className="pt-0">
              {templates.length === 0 ? (
                <div className="text-center py-6 text-sm text-gray-500">등록된 수당/공제 템플릿이 없습니다.</div>
              ) : (
        ...
        <TabsContent value="assignments">
          <AllowanceTemplateAssignmentsSection
      ...
      <Dialog open={copyTarget !== null} onOpenChange={open => { if (!open) setCopyTarget(null); }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <p className="text-xs text-gray-500">
              항목 구성과 금액을 그대로 복사한 새 템플릿을 만듭니다. 원본과는 완전히 독립된 별개 템플릿이며, 복사 후 필요한 부분만 수정하면 됩니다.
            </p>
            <div className="space-y-1.5">
