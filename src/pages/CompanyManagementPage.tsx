              ...
              <div className="space-y-3">
                <div className="flex gap-2 items-center">
                  <div className="flex-1 relative">
                    ...
                    <Input placeholder="이름, 직급, 여권번호, 선원수첩번호로 검색..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 h-9 text-sm" />
                  </div>
                  ...
                  {(selectedOwner !== 'all' || selectedFleet !== 'all' || selectedShip !== 'all' || selectedRank !== 'all' || selectedRankCategory !== 'all' || selectedManningAgency !== 'all' || selectedShipType !== 'all' || minAge || maxAge) && (
                    <Button variant="ghost" size="sm" onClick={clearFilters} className="h-9 text-sm">
                      <X className="w-4 h-4 mr-2" />필터 초기화
                    </Button>
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-4 gap-3 p-3 border rounded-lg bg-muted/50">
                    <div>
                      ...
                      <Select value={selectedOwner} onValueChange={setSelectedOwner}>
                        <SelectTrigger className="h-9 text-sm mt-1"><SelectValue placeholder="전체" /></SelectTrigger>
                    ...
                    <div>
                      ...
                      <Select value={selectedShipType} onValueChange={setSelectedShipType}>
                        <SelectTrigger className="h-9 text-sm mt-1"><SelectValue placeholder="전체" /></SelectTrigger>
                        <SelectContent><SelectItem value="all">전체</SelectItem>{shipTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                    <div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              이 작업은 되돌릴 수 없으며, 선원의 모든 정보가 영구적으로 삭제됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
