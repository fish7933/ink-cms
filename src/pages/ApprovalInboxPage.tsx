...
import { approvalDocumentService, getLeaveDetail, getReferenceDocumentIds, SELF_APPROVE_COMMENT, type LeaveDetail } from '@/services/approval-document.service';
  ...
  const renderDocAction = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {/* 결재권자가 승인/반려 시 남긴 의견 — 문서가 최종 완료된 뒤에도(상태와 무관하게) 계속 보여야
            한다. 기안자 본인 자동승인 시스템 문구(SELF_APPROVE_COMMENT)는 실제 의견이 아니므로 제외. */}
        {selectedDocument.steps.some(s => s.comment && s.comment !== SELF_APPROVE_COMMENT) && (
          <div className="bg-blue-50/60 border border-blue-100 p-3 rounded space-y-2">
            ...
            {selectedDocument.steps.filter(s => s.comment && s.comment !== SELF_APPROVE_COMMENT).map(s => (
