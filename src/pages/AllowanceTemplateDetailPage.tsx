...
import type { AllowanceTemplateWithItems, AllowanceTemplate } from '@/types/allowance';
...
export default function AllowanceTemplateDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { toast } = useToast();
  const { openNewTab, activeTabId, updateTab } = useTabContext();

  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<AllowanceTemplateWithItems | null>(null);
  ...
  const loadData = async (templateId: string, isInitial = false) => {
    ...
    try {
      ...
      const hist = await allowanceService.getTemplateHistory(templateId);
      ...
      const [full, ranksData, ships] = await Promise.all([
        allowanceService.getTemplateWithItems(resolvedId),
        getRanks(),
        getShips(),
      ]);
      setData(full);
      setRanks(sortRanksByDisplayOrder(ranksData));
      if (isInitial && activeTabId && full) updateTab(activeTabId, { title: `수당/공제 템플릿 상세: ${full.name}` });
  ...
  useEffect(() => {
    ...
    window.addEventListener('allowance-template-data-changed', handler);
    return () => window.removeEventListener('allowance-template-data-changed', handler);
  }, [id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleRenew = async () => {
    ...
    setRenewing(true);
    try {
      const newTemplate = await allowanceService.renewTemplate(data.id, renewDate);
      if (!newTemplate) {
      ...
      <Card>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => openNewTab(`/allowance-templates/${data.id}/edit`, `템플릿 수정: ${data.name}`)}
              >
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-1.5"><AllowanceTemplateItemsSummary items={data.items} ranks={ranks} /></div>
          </div>
