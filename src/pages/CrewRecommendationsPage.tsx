          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <div className="relative">
                ...
                <Input
                  placeholder="선원명, 직급, 매닝사, 선박명으로 검색..."
                  value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground">전체 추천</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-muted-foreground">수락</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold">{recommendation.crew_name}</h3>
                      <Badge className={departmentColors[recommendation.department as keyof typeof departmentColors]}>
                        {recommendation.rank_code}
                      </Badge>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span>{recommendation.manning_agency_name}</span>
                      </div>
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Calendar className="h-4 w-4" />
                        <span>생년월일: {new Date(recommendation.crew_birth_date).toLocaleDateString('ko-KR')}</span>
                      </div>
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Calendar className="h-4 w-4" />
                        <span>출국가능: {new Date(recommendation.available_date).toLocaleDateString('ko-KR')}</span>
                      </div>
                      {recommendation.ship_name && (
                        <div className="flex items-center gap-2 text-muted-foreground">
                          <Ship className="h-4 w-4" />
                          <span>{recommendation.ship_name}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium">{recommendation.rank_name}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">희망급여:</span>
                    <span className="ml-2 font-medium">
                      {recommendation.desired_currency} {recommendation.desired_salary.toLocaleString()}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">계약기간:</span>
                    <span className="ml-2 font-medium">{recommendation.desired_contract_months}개월</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">이력서:</span>
                    <span className="ml-2 font-medium">{recommendation.resume_files.length}개 파일</span>
                  </div>
                </div>

                {recommendation.company_name && (
                  <div className="mb-4 text-sm">
                    <span className="text-muted-foreground">선주사:</span>
                    <span className="ml-2 font-medium">{recommendation.company_name}</span>
                    {recommendation.fleet_name && (
                      <>
                        <span className="mx-2 text-muted-foreground">|</span>
                        <span className="text-muted-foreground">선대:</span>
                        <span className="ml-2 font-medium">{recommendation.fleet_name}</span>
                      </>
                    )}
                  </div>
                )}
                ...
                {recommendation.remarks && (
                  <div className="mb-4 p-3 bg-muted rounded-md text-sm">
                    ...
                    <div className="text-muted-foreground whitespace-pre-wrap">{recommendation.remarks}</div>
                  </div>
                )}

                <div className="flex items-center gap-2">
                  {recommendation.resume_files.map((file, index) => (
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleStatusChange(recommendation.id, value as CrewRecommendationWithDetails['status'])}
                  >
