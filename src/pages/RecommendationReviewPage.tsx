...
export default function RecommendationReviewPage() {
  ...
  const submitApproval = async () => {
    ...
    try {
      ...
      await loadAll();
      setApprovalOpen(false); setSelectedRec(null); setApprovalAction(null); setApprovalComment(''); setSaveLineDefault(false);
                ...
                <div className="space-y-4">
                  ...
                  {selectedRec.status === 'pending' && (
                    <div className="flex justify-end gap-2 pt-4 border-t">
                      <Button variant="outline" onClick={() => { setDetailOpen(false); openApproval(selectedRec, 'reject'); }} disabled={!canAp(selectedRec)} className="text-red-600 hover:bg-red-50 disabled:opacity-50">
                        <XCircle className="w-4 h-4 mr-2" />거절
                      </Button>
                      <Button onClick={() => { setDetailOpen(false); openApproval(selectedRec, 'accept'); }} disabled={!canAp(selectedRec)} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50">
