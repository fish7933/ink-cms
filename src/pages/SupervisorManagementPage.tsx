...
export default function SupervisorManagementPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (userData?.role !== 'ship_manager') {
        toast({ title: '접근 권한 없음', description: '선박관리사만 접근할 수 있습니다.', variant: 'destructive' });
        navigate('/dashboard');
        return;
      }
  ...
  const handleAddAssignment = async () => {
    if (!selectedSupervisor) {
      toast({ title: '오류', description: '담당 감독을 선택해주세요.', variant: 'destructive' });
      return;
    }

    let entityId = '';
    if (selectedEntityType === 'owner') {
      if (!selectedOwner) { toast({ title: '오류', description: '선주사를 선택해주세요.', variant: 'destructive' }); return; }
      entityId = selectedOwner;
    } else if (selectedEntityType === 'fleet') {
      if (!selectedFleet) { toast({ title: '오류', description: '플릿을 선택해주세요.', variant: 'destructive' }); return; }
      entityId = selectedFleet;
    } else {
      if (!selectedShip) { toast({ title: '오류', description: '선박을 선택해주세요.', variant: 'destructive' }); return; }
    ...
    try {
      ...
      if (msg.includes('unique') || (error && typeof error === 'object' && 'code' in error && (error as { code: string }).code === '23505')) {
        toast({ title: '오류', description: '이미 동일한 담당 감독 지정이 존재합니다.', variant: 'destructive' });
      } else {
        toast({ title: '오류', description: '담당 감독 지정에 실패했습니다.', variant: 'destructive' });
      }
    } finally {
      setProcessing(false);
    }
  ...
  const handleDeleteAssignment = async () => {
    ...
    try {
      ...
      toast({ title: '성공', description: '담당 감독 지정이 해제되었습니다.' });
      setDeleteDialogOpen(false);
      setSelectedAssignment(null);
      await loadData();
    } catch (error) {
      console.error('Error deleting assignment:', error);
      toast({ title: '오류', description: '담당 감독 해제에 실패했습니다.', variant: 'destructive' });
    } finally {
      setProcessing(false);
    }
  };
  ...
  const resetForm = () => {
    setSelectedSupervisor(''); setSelectedEntityType('owner');
    setSelectedOwner(''); setSelectedFleet(''); setSelectedShip(''); setNotes('');
  };

  const getEntityIcon = (type: 'owner' | 'fleet' | 'ship') => {
    if (type === 'owner') return <Building2 className="w-4 h-4" />;
    if (type === 'fleet') return <Layers className="w-4 h-4" />;
    return <Ship className="w-4 h-4" />;
  };

  const getEntityTypeLabel = (type: 'owner' | 'fleet' | 'ship') => {
    if (type === 'owner') return '선주사';
    if (type === 'fleet') return '플릿';
    return '선박';
  };

  const getFilteredFleets = () => selectedOwner ? fleets.filter(f => f.owner_id === selectedOwner) : fleets;

  const getFilteredShips = () => {
    if (selectedEntityType === 'fleet' && selectedFleet) return ships.filter(s => s.fleet_id === selectedFleet);
    if (selectedEntityType === 'ship' && selectedOwner) return ships.filter(s => s.owner_id === selectedOwner);
    return ships;
  };
  ...
  const groupedAssignments = (assignments || []).reduce((acc, a) => {
    if (!acc[a.supervisor_id]) {
      acc[a.supervisor_id] = { supervisor_name: a.supervisor_name, supervisor_email: a.supervisor_email, assignments: [] };
    }
    acc[a.supervisor_id].assignments.push(a);
    return acc;
            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <span className="text-red-500 text-sm mt-2 block">
                  <AlertCircle className="w-3 h-3 inline mr-1" />이 작업은 되돌릴 수 없습니다.
                </span>
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>취소</AlertDialogCancel>
              <AlertDialogAction onClick={handleDeleteAssignment} className="bg-red-600 hover:bg-red-700" disabled={processing}>
                {processing ? '처리중...' : '해제하기'}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
